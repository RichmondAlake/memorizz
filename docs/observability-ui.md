...
| Runtime spans | model calls, tool calls, failures, duration, and usage |
...
Filter by agent and thread to view the timeline, context/cache tags, tool
health, verified outcomes, and Trace Insights. Insights currently identify
issues such as page/thread mismatch, unverified ownership, missing current-page
grounding, and content attached to an unscoped thread.
